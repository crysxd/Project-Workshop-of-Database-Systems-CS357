<?php
  ...
  $stmt_meal_result = $db_link->prepare("
    SELECT Meal.meal_id_pk, Meal.name name, Meal.price price, Meal.spiciness spicy, AVG( Rating.rating ) rating, COUNT( Rating.rating ) rating_count
    FROM Meal
    ...
    ORDER BY ? ?
    LIMIT ?, ?");
  $stmt_meal_result->bind_param("issii", $restaurant, $order, $direction, $start, $count);
